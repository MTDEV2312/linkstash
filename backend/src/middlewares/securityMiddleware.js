...
// Middleware para rate limiting avanzado (por usuario)
export const advancedRateLimit = (maxRequests = 100, windowMs = 15 * 60 * 1000) => {
  const store = new Map(); // En producción usar Redis
  ...
  return (req, res, next) => {
    ...
    if (Math.random() < 0.01) {
      for (const [k, v] of store.entries()) {
        const recent = v.filter(time => now - time < windowMs);
        if (recent.length === 0) {
          store.delete(k);
        } else {
          store.set(k, recent);
